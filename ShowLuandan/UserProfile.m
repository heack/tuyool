...
-(NSArray*) getFavoriteUsers
{
    ...
    if (self.favoriteUsers == nil) {
        NSLog(@"here2");
        NSString *url_string = [[NSString alloc] initWithFormat:@"http://swinglife.crosslife.com/favorite.php?ownid=%@&online=%@&method=list",self.userId,self.isOnlyShowOnlineUsers?@"1":@"0"];
        NSLog(@"get nearfavoriteusers:%@",url_string);
        NSURL *url = [NSURL URLWithString:url_string];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
        [url_string release];
        [request startSynchronous];
        ...
        if (!error) {
            NSString *response = [request responseString];
            NSLog(@"get nearfavoriteusers with response:%@", response);
            SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
            [self setFavoriteUsers:[jsonParser objectWithString:response]];
            [jsonParser release];
...
{
    NSString *url_string = [[NSString alloc] initWithFormat:@"http://swinglife.crosslife.com/favorite.php?ownid=%@&friendid=%@&method=add",self.userId,targetId];
    NSURL *url = [NSURL URLWithString:url_string];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [url_string release];
    [request startSynchronous];
    NSError *error = [request error];
    if (!error) {
        [self.delegate showMessage:@"Success!"];
    } else {
        [self.delegate showErrorMessage:@"Please Check Your Network Connectoin."];
...
{
    NSString *url_string = [[NSString alloc] initWithFormat:@"http://swinglife.crosslife.com/favorite.php?ownid=%@&friendid=%@&method=delete",self.userId,targetId];
    NSURL *url = [NSURL URLWithString:url_string];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [url_string release];
    [request startSynchronous];
    NSError *error = [request error];
    if (!error) {
        [self.delegate showMessage:@"Remove Success!"];
    } else {
        [self.delegate showErrorMessage:@"Please Check Your Network Connectoin."];
...
{
    NSString *url_string = [[NSString alloc] initWithString:@"http://swinglife.crosslife.com/latestversion.php"];
    NSURL *url = [NSURL URLWithString:url_string];
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [url_string release];
    [request startSynchronous];
    ...
    if (!error) {
        NSString *response = [request responseString];
        SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
        NSDictionary *ver = [jsonParser objectWithString:response];
        [jsonParser release];
        NSString *version = [ver objectForKey:@"v"];
        if (![version isEqualToString:@"1.2"]) {
            NSString *msg = [NSString stringWithFormat:@"There appears to be a newer version %@ of SwingLife available for download. Would you like to get it now?", version];
...
{
    if (buttonIndex == 1) {
        NSURL *url = [NSURL URLWithString:@"http://itunes.apple.com/app/swinglife/id475584720?mt=8"];
